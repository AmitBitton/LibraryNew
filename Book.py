class Book:
    ...
    def add_to_waiting_list(self, name: str, phone_number: str, email: str):
        self._waiting_list.append({"name": name, "phone number": phone_number, "email": email})
        ...
        if num >= self._copies:
            ...
            if num >= self.borrowed_copies:
                ...
                for i in range(0, num):
                    for i in range(1, self._copies + 1):
                        if self._borrowed_copies_status[i] == "No":
                            self._borrowed_copies_status.pop(i)
                            break
